...
max(plot_htmp)
plot_htmp[plot_htmp > 10] <- 10
quantile(plot_htmp)
Heatmap(plot_htmp)
length(heat_genes$Rel)
```
...
dev.off()
#c(head(brewer.puor(100), n = 50), tail(brewer.brbg(100), n = 50))
```
```{r}
length(plot_genes)
```
...
t_subset@meta.data$fig3 <- ifelse(t_subset@meta.data$Annotation_new %in% c("gdT", "MAIT"), 
                                  "Unconv. T", 
                                  ifelse(t_subset@meta.data$Annotation_new %in% c("CD8 mem. 1", "CD8 mem. 2", "CD8 mem. 3", 
                                                                                  "CD8 eff. 1", "CD8 eff. 2"), 
                                         "CD8 EM",
                                         ifelse(t_subset@meta.data$Annotation_new %in% c("CD4 NV", "Th", "Th17", "Treg", "CD4 IFN"), 
                                                "CD4 T", 
                                                t_subset@meta.data$Annotation_new)))
pdf("/g/scb2/zaugg/amathiou/2020Dec_scBM_Tcells/6.ManuscriptFigures/Figure3/UMAP_202208.pdf", width = 3.5, height = 4)
DimPlot(t_subset, group.by = "fig3", cols = c("lightgrey", "#FB9A99", "#ECC978", "#FDBF6F", "#FF7F00","#CAB2D6"), pt.size = 0.05, label = T, repel = T, order = F) + xlim(-5, 5) + theme_void()
dev.off()
```
```{r}
t_subset <- subset(seurat_integrated, subset = Annotation_new %in% c("MLP", "MEP", "NP", "MDP", "pDCs", "cDCs", "preB", "Mono", "pro/pre T"), invert = T)
